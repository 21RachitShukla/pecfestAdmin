...
class Login extends Component {
    ...
    handleSubmit = (event) => {
        ...
        fetch('https://api.pecfest.in/v1/portal/user/coordinator', {
            ...
            this.setState({error: true, status: err.message})
        });
...
export default class App extends Component {
    ...
    handleLogout = () => {
        fetch('https://api.pecfest.in/v1/portal/user/logout', {
            method: 'POST',
            headers: {
                'Authorization': 'Basic ' + btoa(window.localStorage.getItem('sessionKey'))
            }
        }).then(data => data.json())
            .then(res => {
                if (res.ACK === 'SUCCESS') {
